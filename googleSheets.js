...
async function getSheetData(url, sheetName) {
  try {
    ...
    // Lấy LessonIndex ở ô B1 và LessonTitle ở ô B3
    const infoResponse = await sheets.spreadsheets.values.batchGet({
      spreadsheetId,
      ranges: [`${sheetName}!B1`, `${sheetName}!B3`],
    });
    const lessonIndex =
      (infoResponse.data.valueRanges[0].values &&
        infoResponse.data.valueRanges[0].values[0][0]) ||
      "";
    const lessonTitle =
      (infoResponse.data.valueRanges[1].values &&
        infoResponse.data.valueRanges[1].values[0][0]) ||
      "";
...
async function updateComments(url, sheetName, comments) {
  try {
    ...
    const commentColumnIndex = headers.findIndex(
      (header) => header === "Nhận xét về buổi học"
    );
