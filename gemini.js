  ...
  try {
    ...
    const studentInfo = Object.entries(studentData)
      .map(([key, value]) => {
        ...
        return `${key}: ${value || "Học tốt, thái độ tốt, cần phát huy"}`;
      })
      .join("\n        ");

    const prompt = `Hãy viết một đoạn nhận xét ngắn gọn, chuyên nghiệp và khách quan cho học sinh dựa trên thông tin sau:
        ...
        4. Câu văn khách quan, không được mang yếu tố cảm xúc
